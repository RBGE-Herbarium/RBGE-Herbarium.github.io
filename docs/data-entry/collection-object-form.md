...
- **Storage** – enter the storage location of the specimen (e.g. Main Herbarium, Long Store, Spirit Room, Silica Room, Carpological Collection). For physically linked specimens (where there is more than one specimen permanently mounted on a sheet), the lowest barcode should be created as a storage location and used to store all the specimens on the sheet. See the **[Physically Linked](https://github.com/RBGE-Herbarium/RBGE-Herbarium.github.io/blob/main/queries.md#finding-specimens-that-are-physically-linked)** section for more information.
